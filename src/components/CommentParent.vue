 ...
 components: {
  ...
  methods:{
      addComment:function(cont,use,pID,cID){
        if (cont!='')
          this.comments.push({user:use, content:cont, idx:this.comments.length,level:0,parentIdx:-1,parentID:pID,currentID:cID});
          else
      ...
      addReply:function(cont,use,parent,l,pID,cID){
        ...
        {
          var i = parent+1;
          this.comments.push({user:use, content:cont ,idx:i ,level:l,parentIdx:parent,parentID:pID,currentID:cID });
